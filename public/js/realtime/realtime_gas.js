...
.then(() => { // We start building database nodes listeners only when we have devices ids.
    for (let i = 0; i < devicesIds.length; i++) {
        db.ref(`sensor/${devicesIds[i]}`).limitToLast(nbOfElts).on('value', ts_measures => {
            ...
            ts_measures.forEach(ts_measure => {
                ...
                gas[devicesIds[i]].push(ts_measure.val().gas);
            });

            // plotly.js: See https://plot.ly/javascript/getting-started/
            // Temperatures
            let gasTraces = []; // array of plotly temperature traces (n devices => n traces) 
            for (let i = 0; i < devicesIds.length; i++) {
                gasTraces[i] = {
            ...
            let gasData = []; // last plotly object to build
            for (let i = 0; i < devicesIds.length; i++) {
                gasData.push(gasTraces[i]);
            }
